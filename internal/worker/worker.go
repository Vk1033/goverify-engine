...
func (w *Worker) consumeEnroll(ctx context.Context) {
	for {
		...
		if req.CallbackURL != "" {
			w.sendCallback(req.CallbackURL, domain.VerificationResult{
				TransactionID: txnID,
...
func (w *Worker) consumeVerify(ctx context.Context) {
	for {
		...
		if req.CallbackURL != "" {
			w.sendCallback(req.CallbackURL, res)
		}
	}
}

func (w *Worker) sendCallback(url string, payload interface{}) {
	b, err := json.Marshal(payload)
	if err != nil {
		w.logger.Error("failed to marshal callback payload", "error", err)
		return
	}

	req, err := http.NewRequest("POST", url, bytes.NewBuffer(b))
	if err != nil {
		w.logger.Error("failed to create callback request", "error", err)
		return
	}
	...
	if err != nil {
		w.logger.Error("failed to send callback", "error", err, "url", url)
		return
	}
	defer resp.Body.Close()

	if resp.StatusCode >= 400 {
		w.logger.Error("callback returned error status", "status", resp.Status, "url", url)
	} else {
		w.logger.Info("callback sent successfully", "url", url)
	}
}
